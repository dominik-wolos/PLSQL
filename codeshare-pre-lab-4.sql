...
Zad. 235 - Artur Żaczek - OK

DECLARE
    TYPE TREC_EGZAMINATOR IS RECORD (
            IDEGZAMINATOR VARCHAR2(4),
            IMIE          VARCHAR2(15),
            NAZWISKO      VARCHAR2(25),
            LICZBAEGZ     NUMBER
    );
    REGZAMINATOR     TREC_EGZAMINATOR;
    TYPE TCOL_EGZAMINATORZY IS
        TABLE OF TREC_EGZAMINATOR;
        
    COLEGZAMINATORZY TCOL_EGZAMINATORZY := TCOL_EGZAMINATORZY();
    TYPE TREC_OSRODEK IS RECORD (
            IDOSRODEK     NUMBER,
            NAZWAOSRODEK  VARCHAR2(50),
            EGZAMINATORZY TCOL_EGZAMINATORZY
    );
    ROSRODEK         TREC_OSRODEK;
    
    TYPE TCOL_ANALITYKA IS
        TABLE OF TREC_OSRODEK;
    ANALITYKA        TCOL_ANALITYKA := TCOL_ANALITYKA();
    
    CURSOR COSRODKI IS
    SELECT
        O.ID_OSRODEK    AS IDOSRODEK,
        O.NAZWA_OSRODEK AS NAZWAOSRODEK
    FROM
        OSRODKI O
    ORDER BY
        O.ID_OSRODEK;

    CURSOR CEGZAMINATORZY (
        PIDOSRODEK NUMBER
    ) IS
    SELECT
        EG.ID_EGZAMINATOR AS IDEGZAMINATOR,
        EGZ.IMIE          AS IMIE,
        EGZ.NAZWISKO      AS NAZWISKO,
        COUNT(*)          AS LICZBAEGZ
    FROM
             EGZAMINY EG
        JOIN EGZAMINATORZY EGZ ON EGZ.ID_EGZAMINATOR = EG.ID_EGZAMINATOR
    WHERE
        EG.ID_OSRODEK = PIDOSRODEK
    GROUP BY
        EG.ID_EGZAMINATOR,
        EGZ.IMIE,
        EGZ.NAZWISKO
...
BEGIN
    FOR OSRODEK IN COSRODKI LOOP
        ...
        COLEGZAMINATORZY := TCOL_EGZAMINATORZY();
        
        FOR EGZAMINATOR IN CEGZAMINATORZY(OSRODEK.IDOSRODEK) LOOP
            COLEGZAMINATORZY.EXTEND;
            REGZAMINATOR := TREC_EGZAMINATOR(EGZAMINATOR.IDEGZAMINATOR, EGZAMINATOR.IMIE, EGZAMINATOR.NAZWISKO, EGZAMINATOR.LICZBAEGZ
            );

            COLEGZAMINATORZY(COLEGZAMINATORZY.LAST) := REGZAMINATOR;
        END LOOP;

        ROSRODEK := TREC_OSRODEK(OSRODEK.IDOSRODEK, OSRODEK.NAZWAOSRODEK, COLEGZAMINATORZY);
        ANALITYKA(ANALITYKA.LAST) := ROSRODEK;
    END LOOP;

    FOR I IN 1..ANALITYKA.COUNT LOOP
        ROSRODEK := ANALITYKA(I);
        DBMS_OUTPUT.PUT_LINE('============= START Osrodek '
                             || ROSRODEK.IDOSRODEK || ' =============');
        DBMS_OUTPUT.PUT_LINE('Osrodek: '
                             || ROSRODEK.IDOSRODEK
                             || ' ' || ROSRODEK.NAZWAOSRODEK);

        FOR J IN 1..ROSRODEK.EGZAMINATORZY.COUNT LOOP
            REGZAMINATOR := ROSRODEK.EGZAMINATORZY(J);
            DBMS_OUTPUT.PUT_LINE('Egzaminator: '
                                 || REGZAMINATOR.IDEGZAMINATOR
                                 || ' '
                                 || REGZAMINATOR.IMIE
                                 || ' '
                                 || REGZAMINATOR.NAZWISKO
                                 || ', liczba egzaminow: ' || REGZAMINATOR.LICZBAEGZ);

        END LOOP;

        DBMS_OUTPUT.PUT_LINE('============= END Osrodek '
                             || ROSRODEK.IDOSRODEK || ' =============');
    END LOOP;

END;

Zad. 237 -- Dominik Wołos - OK
